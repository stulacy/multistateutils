...
void Event::processEvent(Simulation* sim) {
    ...
    entering_state = sim->get_state(state_entering);
    if (entering_state->is_transient()) {
        //double entry_time = time - sim->get_sim_entry_time(individual_id);
        //double soujourn_time = time - sim->get_previous_state_entry_time(individual_id);
