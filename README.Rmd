...
However, the advantage of the implementation provided by `rdes` is that estimates can be produced from multiple conditions without needing to duplicate the simulations in the same manner as `predict_transitions`.
...
Another feature in `RDES` is a visualization of a predicted pathway through the state transition model, calculated using *dynamic prediction* and provided in the function `plot_predicted_pathway`. It estimates state occupancy probabilities at discrete time-points and displays the flow between them in the manner of a Sankey diagram. 
