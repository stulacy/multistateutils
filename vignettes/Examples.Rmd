...
This is useful for situations where we have a heterogeneous cohort and are interested in group-level metrics, rather than individual ones; for example, the amount of time spent in each state for individuals with certain covariates.
...
The application of multi-state modelling in these contexts is often referred to as **discrete event simulation**.

The `cohort_simulation` function provides this functionality. 

It is specified very similarly to the other functions in this package, requiring:
...
The first rows below shows that every individual enters the system in state 1 at time 0, which is the default behaviour.
...
The simulation below evenly splits patients between starting in the initial state and platelet recovery (*illness*).
...
Likewise, the individuals don't have to enter the simulation at time 0.
The `start_time` parameter is specified in the same way as `start_state`, accepting either a single value or a vector containing a time for each individual. 
The example below shows the case where individuals enter the system every 10 days (in the `ebmt3` dataset this is having a transplant).
...
Sometimes it is appropriate to incorporate a time limit; for example, if we are interested in looking at the cost to the health care provider from treating a particular disease over 10 years.
The `time_limit` argument allows for this specification, by stopping the simulation at the given time.

The model below uses the same incidence model of a transplant every ten days, but now terminates at 10 years, which can be seen by the last state being entered at exactly 10 years.
...
Problem with RNG is get unrealistic event times due to drawing from random number distribution that has been fitted to data that is missing follow-up on patients. 
See below, the oldest patient dies `r sim %>% filter(state == 'death') %>% summarise(m=round(max(time)/365.25)) %>% pull(m)` years after the transplant!
...
To combat this, each of `prediction_transitions`, `length_of_stay`, and `cohort_simulation` allow the option to specify a hard limit after which a patient is considered dead.
This is achieved through 3 arguments:

  - `agelimit`: This is either `FALSE`, which means don't use a limit, or is set to the requested limit if used
  - `agecol`: The column in `newdata` that holds the patient age at entry to the simulation (in the `ebmt3` dataset this is age at transplant.)
  - `agescale`: Often, age is measured in a different time-scale to the rest of the study. For example in many health studies age will be measured in years but study time is on a months or days time-scale. This argument provides the multiplicative factor to be applied to the individual's age to get it on the same time-scale as the study
  
The example below shows how to use this in practice, firstly having to simulate continuous age values for the transplant patients as only binned group are provided with the data set.
...
Can see that the maximum state entry time is now `r sim5 %>% top_n(1, time) %>% mutate(time=round(time/365.25)) %>% pull(time)` years, which means they died at the age of `r sim5 %>% top_n(1, time) %>% mutate(agedead = round((age_cont * 365.25 + time) / 365.25)) %>% pull(agedead)`, showing that the hard limit is working.

**Again, these arguments are in the `predict_transitions` and `length_of_stay` functions too although they are less often useful.**
